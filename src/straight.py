...
def validate_straight(
    ...
    rank_of_max_card_as_int = unique_sorted_raw_rank_values[0]
    rank_of_max_card_as_string = find_key_by_raw_rank_value(rank_of_max_card_as_int)
    ...
    for current_rank, next_rank in zip(
        ...
        if current_rank == (next_rank + 1):
            ...
            if current_n_cards_towards_a_straight == n_cards_in_a_straight:
                straight_found = True
                rank_of_max_card_in_straight_as_int = current_rank - 3
                rank_of_max_card_as_string = find_key_by_raw_rank_value(
                    rank_of_max_card_as_int
                )
                name = f"{hand_name_root}: {rank_of_max_card_as_string} high."
                logger.debug(
                    f"Max card in straight is {rank_of_max_card_in_straight_as_int}."
                )
                top_ranks_in_straight_as_int = [
            ...
            current_n_cards_towards_a_straight = 1
            rank_of_max_card_as_int = next_rank
